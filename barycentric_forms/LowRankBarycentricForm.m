classdef LowRankBarycentricForm
    ...
    methods
        ...
        function CT_init = init_coefs_tensor(obj,CT)
            ...
            if isa(CT,'tensor') || isa(CT,'ktensor') || isa(CT,'ttensor') 
                CT_init = CT; % case where tensors are simply initialized by passed value
            elseif ndims(CT) == length(obj.orders) && all(size(CT) == obj.orders+1)
                CT_init = tensor(CT); % case where a Matlab tensor is passed and needs to be converted to tensor toolbox tensor
            elseif size(CT,1) == prod(obj.orders+1)
                CT_init = tensor(CT,flip(obj.orders)+1); % case where vectorized coefficients are passed
                CT_init = permute(CT_init,obj.num_vars:-1:1); % need to do rearranging to account for Matlabs Kronecker product order
            else
                CT_init = tensor(CT,obj.orders+1); % case where trailing one dimensions are removed from passed Matlab tensor
            end
        end
